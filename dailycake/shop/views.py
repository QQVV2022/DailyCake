...
def sign_up(request):
    if request.method == 'POST':
        ...
        items = Users.objects.create_user(username = user_name, email=email,password=password,is_active=False, create_time=datetime.now(), last_modify_time=datetime.now())
        items.save()
